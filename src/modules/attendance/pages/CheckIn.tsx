...
import { Clock, MapPin, LogIn, LogOut, CheckCircle2, AlertCircle, User } from 'lucide-react';
  ...
  return (
    <div className="max-w-lg mx-auto space-y-5">
      ...
      {coords && coords.latitude !== 0 && (
        <div className="flex items-center gap-2.5 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl px-4 py-3">
          <MapPin className="h-4 w-4 text-indigo-500 flex-shrink-0" />
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Location detected: <span className="font-mono text-gray-900 dark:text-white">{coords.latitude.toFixed(4)}, {coords.longitude.toFixed(4)}</span>
          </p>
        </div>
      )}
